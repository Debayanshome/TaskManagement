...
namespace TaskManagement.Core.Task.Commands.Update
{
    public class Handler : TaskBaseHandler, IRequestHandler<CommandModel, ValidationResult>
    {
        ...
        public Handler(IRepository<Repository.Models.TaskDetail> taskDetailRepository, IMapper mapper, ILogger<Handler> logger, IRepository<Repository.Models.Employee> employeeRepository) : base(employeeRepository)
        {
        ...
        public async Task<ValidationResult> Handle(CommandModel request, CancellationToken cancellationToken)
        {
          ...
          await  AssignValuesAsync(request, taskDetail, cancellationToken);
            ...
            _logger.LogInformation("Successfully updated the task with Id: {Id}", taskDetail.Id);
